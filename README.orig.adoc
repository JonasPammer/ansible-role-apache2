...
the `community.general`-collection (containing the `pkg5` module) must be installed on the Ansible controller.

When targeting Suse-based systems,
`community.general`-collection (containing the `zypper` module) must be installed on the Ansible controller.
